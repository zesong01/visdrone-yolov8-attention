...
class BboxLoss(nn.Module):
    ...
    def __init__(self, reg_max: int = 16, iou_type: str = "CIoU"):
        ...
        Args:
            ...
            iou_type (str): IoU loss type - one of 'CIoU', 'EIoU', 'WIoU'.
        """
        super().__init__()
    ...
    def forward(
        ...
        if isinstance(iou, tuple):
            # WIoU returns (iou, focusing_weight)
            iou_val, focusing_weight = iou
            loss_iou = ((1.0 - iou_val) * focusing_weight * weight).sum() / target_scores_sum
        else:
...
class v8DetectionLoss:
    ...
    def __init__(self, model, tal_topk: int = 10, tal_topk2: int | None = None):  # model must be de-paralleled
        ...
        self.bbox_loss = BboxLoss(m.reg_max, iou_type=os.environ.get("YOLO_IOU_TYPE", "CIoU")).to(device)
