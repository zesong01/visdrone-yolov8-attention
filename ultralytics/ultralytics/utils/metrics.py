...
def bbox_iou(
    ...
    # IoU
    iou = inter / union
    if CIoU or DIoU or GIoU or EIoU or WIoU:
        cw = b1_x2.maximum(b2_x2) - b1_x1.minimum(b2_x1)  # convex (smallest enclosing box) width
        ch = b1_y2.maximum(b2_y2) - b1_y1.minimum(b2_y1)  # convex height
        if CIoU or DIoU or EIoU or WIoU:  # Distance, Complete, Efficient, or Wise IoU
            ...
            if EIoU:  # Efficient IoU https://arxiv.org/abs/2101.08158
                # Replaces CIoU's aspect ratio penalty with explicit w/h penalties
                rho_w2 = (w2 - w1).pow(2)  # width difference squared
                rho_h2 = (h2 - h1).pow(2)  # height difference squared
                cw2 = cw.pow(2) + eps       # convex width squared
                ch2 = ch.pow(2) + eps       # convex height squared
                return iou - (rho2 / c2 + rho_w2 / cw2 + rho_h2 / ch2)  # EIoU
            if WIoU:  # Wise IoU v1 https://arxiv.org/abs/2301.10051
                # WIoU uses a focusing mechanism based on the ratio of
                # actual distance to "wise" (outlier-aware) distance.
                # WIoU v1: returns (iou_loss, focusing_weight) tuple
                # The focusing weight downweights outliers (hard examples)
                # that may have noisy annotations.
                wise_iou_loss = 1.0 - iou + rho2 / c2  # base DIoU loss
                with torch.no_grad():
                    # Focusing weight: exp((rho2 / c2) normalized)
                    # This creates a dynamic weight that reduces gradient
                    # contribution from geometric outliers
                    beta = rho2.detach() / c2.detach()
                    focusing_weight = torch.exp(beta - beta.mean())
                return iou, focusing_weight  # tuple: (iou, weight)
